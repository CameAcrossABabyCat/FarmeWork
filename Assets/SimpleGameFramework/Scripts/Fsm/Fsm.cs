...
namespace SimpleGameFramework
{
    ...
    public class Fsm<T> : IFsm where T : class
    {
        ...
        public Fsm(string name, T owner, params FsmState<T>[] states)
        {
            ...
            {
                if (state == null)
                    Debug.Log("要添加进状态机的状态为空");
                string stateName = state.GetType().FullName;
                if (m_States.ContainsKey(stateName))
                    Debug.Log("要添加进状态机的状态已存在:" + stateName);
        ...
        public FsmState<T> GetState(Type stateType)
        {
            ...
            if (!typeof(FsmState<T>).IsAssignableFrom(stateType))
                Debug.Log("要获取的状态" + stateType.FullName + "没有直接或间接的实现" + typeof(FsmState<T>).FullName);
        ...
        {
            if (CurrentState != null)
                Debug.Log("当前状态机已开始,无法再次开始");
            if (type == null)
                Debug.Log("要开始的状态为空,无法开始");
